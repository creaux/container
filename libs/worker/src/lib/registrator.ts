import { GlobalWindow, Window, EventTarget } from 'happy-dom-without-node';
import type { IOptionalBrowserSettings } from 'happy-dom-without-node';
...
export default class GlobalRegistrator {
  private static registered: {
    [key: string]: PropertyDescriptor | null;
  } | null = null;
    ...
    for (const key of Object.keys(this.registered)) {
      if (this.registered[key] !== null) {
        Object.defineProperty(
          global,
          key,
          this.registered[key] as PropertyDescriptor,
        );
      } else {
        delete global[key];
      }
